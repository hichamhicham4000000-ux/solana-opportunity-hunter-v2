...
def _calculate_confidence(
    ...
    smart_money_quality = (
        _number(smart_money.confidence, 0.0)
...
def _calculate_opportunity_score(
    ...
    smart_money_score = (
        _number(smart_money.score, 50.0)
        if smart_money is not None
        else 50.0
    )
...
def _decision_from_scores(
    ...
    if (change_5m > 50 or change_1h > 150) and opportunity_score < 80:
        return WATCH

    if manipulation is not None:
        if manipulation.risk_level == "CRITICAL":
            return WATCH
        if manipulation.risk_level == "HIGH" and opportunity_score < 85:
            return WATCH

    if market is not None and market.market_score < 45 and opportunity_score < 75:
        return WATCH

    if opportunity_score >= 82 and security_score >= 75 and confidence >= 65:
        return STRONG_OPPORTUNITY

    if opportunity_score >= 68 and security_score >= 60 and confidence >= 55:
        return OPPORTUNITY
...
def _build_reasons(
    ...
    if ranked.rank_score >= 70:
        reasons.append("المرشح يحتل ترتيبًا مرتفعًا ضمن فرص الاكتشاف")
    ...
    if market is not None:
        if market.market_score >= 70:
            reasons.append("تحليل السوق الحالي إيجابي نسبيًا")
        if market.momentum_score >= 70:
            reasons.append("الزخم إيجابي عبر أكثر من إطار زمني")
        if market.pressure_score >= 80:
            reasons.append("ضغط الشراء إيجابي")
        if market.breakout_score >= 75:
            reasons.append("توجد إشارة مبكرة لاحتمال توسع الحركة")

    if smart_money is not None:
        if smart_money.state == "STRONG":
            reasons.append("إشارة Smart Money قوية نسبيًا")
        elif smart_money.state == "POSITIVE":
            reasons.append("إشارة Smart Money إيجابية")
        elif smart_money.state == "NEUTRAL":
            reasons.append("إشارة Smart Money محايدة")
        elif smart_money.state == "WEAK":
            reasons.append("إشارة Smart Money ضعيفة")
        elif smart_money.state == "NONE":
            reasons.append("لا توجد محفظة Smart Money معروفة ضمن البيانات المتاحة")
        reasons.extend(smart_money.reasons[:3])

    if manipulation is not None and manipulation.manipulation_score < 15:
        reasons.append("لا توجد إشارات تلاعب سوقي قوية ضمن البيانات المتاحة")

    if opportunity_score >= 80:
        reasons.append("توافق جيد بين عدة طبقات مستقلة من الأدلة")
...
def _build_warnings(
    ...
    warnings.extend(security_decision.warnings[:6])
    warnings.extend(ranked.warnings[:6])
    ...
    if manipulation is not None:
        ...
        warnings.extend(manipulation.limitations[:2])
...
class DeepIntelligenceEngine:
    ...
    async def analyze(
        ...
        # Market Intelligence uses the already discovered Candidate data.
        market = market_intelligence.analyze(candidate)

        # Manipulation analysis uses Market Intelligence as additional context.
